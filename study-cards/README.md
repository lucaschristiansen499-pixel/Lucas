# Study Cards

Study cards produced from AI Pocket lecture transcripts, formatted the same way Lucas's Notecard tool (`notecard/lucas_notecard.html`) formats them: TL;DR, terms, steps, worked examples, watch-outs, a homework checklist, and a "double-check this" section for anything the source transcript left unclear.

| Date | Class | File |
|---|---|---|
| 2026-09 | Religion | [2026-09-religion-pharisees-sadducees.md](2026-09-religion-pharisees-sadducees.md) |
| 2026-09 | Algebra | [2026-09-algebra-properties-and-expressions.md](2026-09-algebra-properties-and-expressions.md) |

Add new cards here as more transcripts come in — same filename pattern: `YYYY-MM-subject-short-topic.md`.
